...
function App() {
  ...
  const [menuStateLoading, setMenuStateLoading] = useState(true)
  ...
  return (
    <main
      aria-label={`Cardápio digital ${restaurantProfile.name}`}
      className="min-h-screen overflow-x-hidden bg-slate-100 text-slate-950 md:grid md:place-items-center md:px-6 md:py-8"
    >
      <div
        className={`brand-theme fixed inset-0 h-[100dvh] w-[100dvw] max-w-none overflow-hidden md:relative md:mx-auto md:h-[932px] md:w-full md:max-w-[430px] md:rounded-[28px] md:shadow-2xl md:shadow-slate-300/80 ${
          screen === 'entrada' ? 'bg-[#45150d]' : 'bg-white'
        }`}
        style={buildThemeStyle(restaurantProfile)}
      >
        {menuStateLoading && <MenuLoadingScreen />}
...
function MenuLoadingScreen() {
  return (
    <section className="fixed inset-0 z-[500] grid place-items-center bg-white md:absolute" role="status" aria-live="polite" aria-label="Carregando cardápio">
      <div className="grid justify-items-center gap-4 px-8 text-center">
        <span className="size-11 animate-spin rounded-full border-[3px] border-slate-200 border-t-[#4b160e]" aria-hidden="true" />
        <div>
          <p className="text-base font-semibold text-slate-800">Carregando cardápio…</p>
          <p className="mt-1 text-xs text-slate-500">Preparando as informações do restaurante.</p>
        </div>
      </div>
    </section>
  )
